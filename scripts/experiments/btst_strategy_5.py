"""BTST Strategy #5 — Donchian 20-Day Breakout
BUY  = Close is highest close in 20 days
SELL = Close is lowest close in 20 days
...
def backtest(symbol, period="6mo"):
    ...
    df["High_20_Close"] = df["Close"].rolling(20).max()
    df["Low_20_Close"] = df["Close"].rolling(20).min()
    df["Sig_Buy"] = df["Close"] == df["High_20_Close"]
    df["Sig_Sell"] = df["Close"] == df["Low_20_Close"]
    trades = []
...
if __name__ == "__main__":
    print("=" * 80)
    print("  BTST Strategy #5 — Donchian 20-Day Breakout")
    print("  BUY = Close = 20-day high close | SELL = Close = 20-day low close")
