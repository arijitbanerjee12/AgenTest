"""BTST Strategy #3 — RSI Mean Reversion
BUY  = RSI(14) < 30 + close > prev close (first green after oversold)
SELL = RSI(14) > 70 + close < prev close (first red after overbought)
Exit = next-day close · No SL · No target
"""
...
def backtest(symbol, period="6mo"):
    ...
    rs = avg_g / avg_l
    df["RSI"] = 100 - (100 / (1 + rs))
    df["Sig_Buy"] = (df["RSI"] < 30) & (df["Close"] > df["Close"].shift(1))
    df["Sig_Sell"] = (df["RSI"] > 70) & (df["Close"] < df["Close"].shift(1))
    trades = []
    for i in range(len(df) - 1):
        t = df.iloc[i]; nx = df.iloc[i + 1]
        if t["Sig_Buy"]: action = "BUY"
        elif t["Sig_Sell"]: action = "SELL"
        else: continue
...
if __name__ == "__main__":
    print("=" * 80)
    print("  BTST Strategy #3 — RSI Mean Reversion")
    print("  BUY = RSI(14) < 30 + green candle | SELL = RSI(14) > 70 + red candle")
